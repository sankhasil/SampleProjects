...
public final class CommonUtils {
  ...
  /**
   * De-skew a image provided based on minimum de skew threshold. It replaces original with
   * de-skewed image Example threshold would be 0.5d
   * 
   * @param image {@link BufferedImage}
   * @param minimumDeskewThreshold {@link Double}
   * @return de-skewed image {@link BufferedImage}
   */
  public static BufferedImage deskewImage(BufferedImage image, double minimumDeskewThreshold) {
    ...
    if ((imageSkewAngle > minimumDeskewThreshold || imageSkewAngle < -(minimumDeskewThreshold))) {
      image = ImageUtil.rotate(image, -imageSkewAngle, image.getWidth() / 2, image.getHeight() / 2);
    }
    return image;
  }
  ...
  /**
   * Outputs a list of words which texts were not empty after replacing all "\n" with spaces, then
   * trimming.
   *
   * @param wordList
   * @return
   */
  public static List<Word> prettifyWords(List<Word> wordList) {
    List<Word> outputWords = wordList.parallelStream().map(word -> {
      String text = word.getText().replace("\n", " ").trim();
      if (StringUtils.isNotBlank(text)) {
        text = CommonUtils.tailingColonRemoval(CommonUtils.leadingColonRemoval(text));
        return new Word(text, word.getConfidence(), word.getBoundingBox());
      }
      return null;
