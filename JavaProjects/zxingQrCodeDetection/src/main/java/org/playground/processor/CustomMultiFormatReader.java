 ...
 * 
 */
package org.playground.processor;
...
public class CustomMultiFormatReader implements Reader {
	...
	public List<Result> decodeAllCodes(BinaryBitmap image) throws NotFoundException {
		if(hints == null) {
			setHints(null);
		}
		if (readers != null) {
			List<Result> decodedList = new LinkedList<>();
			for (Reader reader : readers) {
				try {
					decodedList.add(reader.decode(image, hints));
				} catch (ReaderException re) {
					// continue
				}
			}
